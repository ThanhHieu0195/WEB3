	...
	require_once "../../helper/helper.php";
 	...
 	switch ($METHOD) {
 		...
 		case 'POST':
 			# code...
 			$OUTPUT['METHOD'] = "POST";
 			// them danh sach khach hang
 			if (isset($_POST['arr'])) {

 				$madatcho = $_REQUEST['madatcho'];
	 			$adanhxung = $_REQUEST['adanhxung'];
	 			$aho = $_REQUEST['aho'];
	 			$aten = $_REQUEST['aten'];

	 			if (isset($madatcho) && isset($adanhxung) && isset($aho) && isset($aten)) {

	 				http_response_code(200);
	 				for ($i=0; $i < count($adanhxung); $i++) { 
	 					# code...
	 					$danhxung = $adanhxung[$i];
	 					$ho = $aho[$i];
	 					$ten = $aten[$i];
